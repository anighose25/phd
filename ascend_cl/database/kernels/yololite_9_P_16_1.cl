__kernel void pooling_9( __global const float* b8, __global float* b9)
{
	...
	if(dp_0< 128*1*1)
	{
		...
		localId_P = dp_0 % (1*1);
		outputRow_P = localId_P / 1;
		outputCol_P = localId_P % 1;
		image2dIdx_P = dp_0 / (1*1);
		...
		inputImageOffset  = (n_P*128 + plane_P)*2*2;
		poolInputOffset = inputImageOffset + inputRow_P * 2 + inputCol_P;
		maxValue = b8[ poolInputOffset ];

		process = (inputRow_P + 1 < 2) && (inputCol_P + 1 < 2);

		if(process)
		{
			data0_P = vload2(0,(__global const float *)b8+poolInputOffset);
			data1_P = vload2(0,(__global const float *)b8+poolInputOffset+2);

			data0_P = fmax(data0_P,data1_P);
			maxValue = fmax(data0_P.s0,data0_P.s1);
		}

		if(outputRow_P<1 && outputCol_P<1)
			b9[dp_0] = maxValue;

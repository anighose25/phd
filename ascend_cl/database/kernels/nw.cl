...
nw_kernel(__global int  * reference_d,            //0 input - BS+1 x BS+1
           ...
           __global int  * upper_row_d,           //2 i - BS 
           __global int  * left_col_d,            //3 i - BS 
           __global int * top_left_d,             //4 i - 1
           ...
           int offset_r,                          //12 0
           int offset_c,                           //13 0
		   __global int  * upper_row_d_o,         //14 output - BS 
           __global int  * left_col_d_o,          //15 output - BS 
           __global int * top_left_d_o           //16 output - 1
    )
{  
   ...
   for ( int ty = 0 ; ty < BLOCK_SIZE ; ty++)
    ...
    upper_row_d_o[tx] = SCORE((BLOCK_SIZE), (tx+1));
    left_col_d_o[tx] = SCORE((tx+1), (BLOCK_SIZE));
    
    if (tx == 0){
		top_left_d_o[0] = SCORE(tx, 0);
	}

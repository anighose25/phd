...
def make_ffc_kernel(kernel_number,m1,n1,p1,partition,TS=8,wpt=1):
    return "{} FFCv.json {}\"m1\":{},\"p1\":{},\"n1\":{},\"TS\":{},\"wpt\":{},\"n_chunks\":1,\"partition\":{}{}\n".\
...
total_layers = int(sys.argv[2])
size = 2**(int(sys.argv[1]))
part=10
for i in range(total_layers):
    k,d = make_one_head(N=size,W=size,Q=size,base=8*i,partition=part)
    kernels += k
    deps += d
    if i<total_layers-1:
        #deps.append("{} {}-{} {}\n".format(i*9+7,2,(i+1)*9+8,0))
        deps.append("{} {}-{} {}\n".format(i*8+7,2,(i+1)*8,0))
        deps.append("{} {}-{} {}\n".format(i*8+7,2,(i+1)*8+1,0))
        deps.append("{} {}-{} {}\n".format(i*8+7,2,(i+1)*8+2,0))
    

with open("/home/odroid/ResearchTools/ascend/database/dags/transformer_dag.graph","w") as f:

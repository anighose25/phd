...
FreeTreeAllocator::FreeTreeAllocator(const std::size_t size) : Allocator(size) {
    ...
    static std::string message =
        ...
        std::to_string(sizeof(RBTree::Node) * 2 + rootNodePadding) +
        " bytes for an allocator with atleast " +
        std::to_string(sizeof(RBTree::Node) - sizeof(Header)) +
        " bytes of free space";
    assert(size >= sizeof(RBTree::Node) * 2 + rootNodePadding &&
           message.c_str());
...
void *FreeTreeAllocator::Allocate(const std::size_t size,
    ...
    std::size_t padding = size + sizeof(Header) < sizeof(RBTree::Node)
                              ? sizeof(RBTree::Node) - sizeof(Header) - size
                              : 0;
    ...
    if (node->m_Value >= size + padding + sizeof(RBTree::Node)) {
        RBTree::Node *splittedNode = reinterpret_cast<RBTree::Node *>(
            reinterpret_cast<char *>(node) + sizeof(Header) + size + padding);
        splittedNode->m_Value =
            node->m_Value - (size + padding + sizeof(Header));
        m_Tree.Insert(splittedNode);
        std::size_t *nextBlockAddress = reinterpret_cast<std::size_t *>(
            reinterpret_cast<char *>(splittedNode) + sizeof(Header) +
            splittedNode->m_Value);
        if ((std::size_t)nextBlockAddress <=
            (std::size_t)m_StartAddress + m_Size - sizeof(std::size_t)) {
    ...
    *reinterpret_cast<std::size_t *>(reinterpret_cast<char *>(header) +
                                     sizeof(Header) + header->m_Size) = 0;

    return reinterpret_cast<char *>(node) + sizeof(Header);
}

void FreeTreeAllocator::Deallocate(void *ptr) {
    Header *header = reinterpret_cast<Header *>(reinterpret_cast<char *>(ptr) -
                                                sizeof(Header));
...
void FreeTreeAllocator::Init() {
    ...
    m_Tree.Init(nil);
    void *currentAddress = reinterpret_cast<RBTree::Node *>(
        reinterpret_cast<char *>(m_StartAddress) + sizeof(RBTree::Node) +
        sizeof(Header));
    ...
    RBTree::Node *root = reinterpret_cast<RBTree::Node *>(
        reinterpret_cast<char *>(currentAddress) - sizeof(Header));
    root->m_Value = reinterpret_cast<char *>(m_StartAddress) + m_Size -
                    reinterpret_cast<char *>(root) - sizeof(Header);
    m_Tree.Insert(root);
}

void FreeTreeAllocator::Coalescence(RBTree::Node *curr) {
    RBTree::Node *next = reinterpret_cast<RBTree::Node *>(
        reinterpret_cast<char *>(curr) + sizeof(Header) + curr->m_Value);
    if (((std::size_t)next < (std::size_t)m_StartAddress + m_Size) &&
    ...
    if (curr->m_PrevSize != 0) {
        RBTree::Node *prev = reinterpret_cast<RBTree::Node *>(
            reinterpret_cast<char *>(curr) - curr->m_PrevSize);
        m_Tree.Remove(prev);
        prev->m_Value += curr->m_Value + sizeof(Header);
        m_Tree.Insert(prev);
        std::size_t *nextBlockAddress = reinterpret_cast<std::size_t *>(
            reinterpret_cast<char *>(prev) + sizeof(Header) + prev->m_Value);
        if ((std::size_t)nextBlockAddress <=
            (std::size_t)m_StartAddress + m_Size - sizeof(std::size_t)) {
...
std::size_t FreeTreeAllocator::GetRootNodePadding() {
    void *currentAddress =
        reinterpret_cast<RBTree::Node *>(sizeof(RBTree::Node) + sizeof(Header));
    void *nextAddress = currentAddress;
    std::size_t space =
        sizeof(RBTree::Node) * 3 - sizeof(Header) - sizeof(RBTree::Node);
    std::align(alignof(std::max_align_t), sizeof(std::max_align_t), nextAddress,
               space);

    ...
    while (not all_processed(SE.global_dags)):
        ...
        if (SE.nCPU == 0 and SE.mGPU == 0):
            ...
            if (not SE.frontier.empty()):
                ...
                min_rank=task_object.get_first_kernel().rank
                task_rank_threshold=min_rank
                
                # Get task with earliest local deadline and calculate task_rank_threshold
                #####################################################
                for i in range(0, len(SE.frontier.queue)):
                    
                    current_task = SE.frontier.queue[i]
                    task_rank =  current_task.get_first_kernel().rank
                    if task_rank < min_rank:
                        task_rank_threshold = min_rank
                        min_rank = task_rank
                        task_index_in_queue = i
                    elif task_rank_threshold > task_rank and task_rank != min_rank:
                        task_rank_threshold = task_rank
                ...
                for key in ['cpu','gpu']:
                    for i in ready_times[key].keys():
                        task_time,d = dag.get_allowable_fused_timing_and_depth(dispatching_task_component,key,task_rank_threshold)

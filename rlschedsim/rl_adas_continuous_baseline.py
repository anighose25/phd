...
def parse_arg(args=None):
    parser = argparse.ArgumentParser(
        description='Baseline Scheduler')
    ...
    parser.add_argument('-dnn', '--dnn_type',
                        help='Which DNN graph file to use: 1 --> Normal 2 --> Reduced',
                        default='2')
        ...
        if (SE.nCPU == 0 and SE.mGPU == 0):
            ...
            if (not SE.frontier.empty()):
                ...
                for i in range(0, len(SE.frontier.queue)):
                    ...
                    if SE.dags[current_task.dag_id].deadline < SE.dags[task_object.dag_id].deadline:
                        task_object = SE.frontier.queue[i]
                ...
                task_time['cpu']=ex_cpu[task_name]
                task_time['gpu']=ex_gpu[task_name]
                ...
                for key in ['cpu','gpu']:
                    for i in ready_times[key].keys():
                        if min_ready_time > ready_times[key][i] + task_time[key]:
                            dispatch_device = key
                            device_id = i
                            min_ready_time = ready_times[key][i] + task_time[key]

                cpu = -1
                gpu = -1
                print "Selected ", dispatch_device, device_id
...
if __name__ == '__main__':
    ...
    task_periods = generate_task_set_periods(period_configuration)
    ...
    # ft = open("Results/baseline_test/Acc.stats",'a+')
    # ft.write(str(float(pos) /(pos+neg))+"\n")
    # ft.close()

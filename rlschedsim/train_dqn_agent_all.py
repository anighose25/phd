...
def parse_arg(args=None):
    parser = argparse.ArgumentParser(
        description='RL DQN With Experience Replay Training Module', formatter_class=RawTextHelpFormatter)
    ...
    parser.add_argument('-m', '--model',
                        ...
                        default='test.pth')
    parser.add_argument('-r', '--rewards',
                        help='Dynamic Rewards Filename',
                        default='sample_rewards.stats')
...
8 -> Prioritized Replay = Yes \t Double Q Learning = Yes \t Dueling = Yes\n""",
                        default='1')
    parser.add_argument('-i', '--init_type',
                        help="""Type of Initialisation\n
...
2 -> Cross Entropy\n""",
    ...
    parser.add_argument('-bn', '--batch_norm',
                        ...
                        default='1')
    parser.add_argument('-tbf', '--tensorboard_folder',
                        help='Tensorboard Folder')
    parser.add_argument('-tbs', '--tensorboard_scalar',
                        help='Tensorboard Scalar Name')
...
tbf = args.tensorboard_folder
tbs = args.tensorboard_scalar

# d1 -> Double Q Learning
# d2 -> Dueling Q Learning
d1 = False
d2 = False
per = False
...
#DQNAgent(cfg,num_states,num_actions,replay_size,***PER***,***DUEL***,***DDQN***,environment_params)

initn = args.init_type
act = args.activation
...
print "Batch Normalisation ->",bnorm
DQNAgentObject = DQNAgent(cfg,num_states,num_actions,replay_size,per,d2,d1,initn,act,l_type,lr,bnorm,environment_params)

avg_time = 0
count_for_time = 0

writer = SummaryWriter("Results/" + tbf + "/" + tbs)
...
def schedule_workflows(SE, dags,periods=None):


    # print "Schedule Engine for Task Sets Initiated"
    TransitionMap = {}
    rates, period_values=SE.generate_periodic_schedule(dags, periods)
    SE.adas=True
    ...
    while (not all_processed(SE.global_dags)):
        ...
        if (SE.nCPU == 0 and SE.mGPU == 0):
            ...
            if is_component_finished(c,g):
                ...
                # print "AAAA"
                loss_val = DQNAgentObject.optimize_model()
                # ================================================================== #
                #                        TensorboardX Logging                         #
                # ================================================================== #
                name_string = "loss_" + tbs
                if loss_val >= 0:
                    writer.add_scalar(name_string, loss_val, global_counter)
                    global_counter += 1
            ...
            if (not SE.frontier.empty()):
                ...
                # action=DQNAgentObject.select_action(state_tensor)
                ##TEST##
                action,time_taken=DQNAgentObject.select_action(state_tensor)
                if time_taken > 0:
             ...
             #    if per:
             #      DQNAgentObject.memory.push(DQNAgentObject.memory.tree.get_max_prio(), transition_tuple)
                # else:
                # DQNAgentObject.memory.push(transition_tuple)
...
if __name__ == '__main__':
    ...
    TARGET_UPDATE = int(args.numruns)
    model_file = "Results/" + args.model
    rewards_file = "Results/" + args.rewards
    statistics_file = "Results/" + args.stats
    period_configuration = args.period
    print " "
    print model_file, rewards_file, statistics_file
    global_map = {}
    global_file_list = open("global_map.txt", "r").readlines()
    global_map = make_dict(global_file_list)
    ...
    temp_counter = 0
    for i_epoch in range(int(args.numepochs)):
        for i_episode in range(num_episodes):

            reward_file = open(reward_filename,'a')
            ...
            # DQNAgentObject.optimize_model()
            ##Test##
            if (temp_counter+1)%100 == 0:
                print "Count =",temp_counter+1
            temp_counter += 1
            # print "CCCC"
            loss_val = DQNAgentObject.optimize_model()
            # ================================================================== #
            ...
            if i_episode % TARGET_UPDATE == 0:
                DQNAgentObject.target_net.load_state_dict(DQNAgentObject.policy_net.state_dict())
            rewards = DQNAgentObject.rewards_history
            mean = np.mean(np.asarray(rewards))
            total= np.sum(np.asarray(rewards))
            # print "REWARD", mean, total 
            DQNAgentObject.rewards_history=[]
            mean_rewards.append(mean)
            epid = i_epoch * num_episodes + i_episode
            reward_string = str(epid)+","+str(mean)+"\n"
            lateness_string = str(i_episode)+","+str(avg_lateness)+"\n"
            reward_file.write(reward_string)
            reward_file.close()
            
            # command = "scp "+reward_filename + " anirban@10.5.19.217:~/Programs/"
            # os.system(command)
            DAGTransitionMap.clear()
        model_file_epoch = model_file[:-4]+"_"+str(i_epoch)+".pth"
        torch.save(DQNAgentObject.target_net.state_dict(),model_file_epoch)    
        
    torch.save(DQNAgentObject.target_net.state_dict(),model_file)
    
    print >> f, "reward=",mean_rewards
    print >> f, "tardiness=",tardiness_values
    print >> f, "deadlines_missed=",deadlines_missed_values
    print >> f, "lateness=",lateness_values
    print >> f, "avg_makespan=",avg_makespan_values
    print >> f, "percent_lateness=",percent_lateness_values
    print >> f, "period_configurations=",period_values
    f.close()

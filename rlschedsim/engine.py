...
class ScheduleEngine(object):
    ...
    def get_dag_prt(self, rank_name):
        ...
        for tc in self.frontier.queue:
            ...
            # print "JOB, DAG", job_id, tc.dag_id
            # print tc.rank_values
            current_value = tc.rank_values[rank_name]
            # print "FRONTIER",job_id,current_value
        ...
        for key in ['cpu','gpu']:
            for executing_task in self.currently_executing[key]:
                task, dag, device_id = executing_task
                job_id = dag.job_id
                current_value = task.rank_values[rank_name]
                # print "CURRENTLY EXECUTING",job_id,current_value
                if job_id in dag_prt.keys():
                    dag_prt[job_id] = max(dag_prt[job_id],current_value)    
                else:
                    dag_prt[job_id] = current_value    
        ...
        for key in ['cpu', 'gpu']:
            for tc in self.dispatch_queue[key]:
                job_id = tc.get_first_kernel().job_id
                current_value = tc.get_component_wcet_rank()
                # print "DISPATCH QUEUE", job_id,current_value,tc
                if job_id in dag_prt.keys():
                    dag_prt[job_id] = max(dag_prt[job_id],current_value)    
                else:
                    dag_prt[job_id] = current_value    
    ...
    def get_dag_deadline_requirements(self):
        dag_dr = {}
        ce_flag = {}
        f_flag = {}
        # print "Number of JOBS", self.num_jobs
        ...
        for tc in self.frontier.queue:
            ...
            job_id = self.dags[tc.dag_id].job_id
            deadline = self.dags[tc.dag_id].deadline
            
            # print "JOB, DAG", job_id, tc.dag_id
            # print tc.rank_values
            current_value = deadline - self.time_stamp 
            # print "FRONTIER",job_id,current_value
            if job_id in dag_dr.keys():
                dag_dr[job_id] = current_value    
                ce_flag[job_id] = 1

            # print "DEADLINE:FRONTIER:", current_value,job_id
        ...
        for key in ['cpu','gpu']:
            for executing_task in self.currently_executing[key]:
                ...
                current_value = deadline - self.time_stamp 
                # print "DEADLINE:CURRENTLY EXECUTING",job_id,current_value
                if job_id in dag_dr.keys():
                    if ce_flag[job_id] !=1:
                        dag_dr[job_id] = current_value    
                        f_flag[job_id] = 1
        ...
        for key in ['cpu', 'gpu']:
            for tc in self.dispatch_queue[key]:
                ...
                deadline = self.dags[tc.dag_id].deadline
                current_value = deadline - self.time_stamp 
                # print "DEADLINE:DISPATCH QUEUE", job_id,current_value
                if job_id in dag_dr.keys():
                    if ce_flag[job_id]!=1 and f_flag[job_id]!=1:
                        dag_dr[job_id] = current_value    
        ...
        dr_values = []
        # print dag_prt
        for i in sorted(dag_dr.keys()):
            dr_values.append(dag_dr[i])
        # print "DEADLINE:", dr_values
        return dr_values

            
    def generate_periodic_schedule(self, dags, periods=None,episode=-1):
        import fractions
        ...
        # print "ARRIVAL",arrival_instances
        dag_id = 0
        if episode >=0:
        ...
        for d, release in arrival_instances:
            ...
            dag=DC.create_adas_job_from_file_with_fused_times(input_file,dag_id,self.global_map,self.ex_map,job_id,release,d.period,d.tolerance)
            if episode >= 0:
    ...
    def update_finished_task_set(self, executing_task, dev_type):
        ...
        finish_time = task.finish_time # finish time added
        ...
        # print "SCHEDULE: Finishing Task", task.id, "of fused kernel", task_component.get_kernel_ids(), "of DAG", dag.dag_id, "at", self.time_stamp," FINISH TIME", task.finish_time
        ...
        if task_component.number_of_tasks == 0:
            ...
            if dev_type == "gpu":
                ...
                if len(self.dispatch_queue['gpu']) > 0:
                    tc = self.dispatch_queue['gpu'].popleft()
                    self.mGPU += 1
                    self.dispatch_task_set(-1,device_id,tc,self.dags[tc.dag_id])
                    # print "SCHEDULE: Dispatching from GPU Queue" + str(list(tc.get_kernel_ids())) + " of DAG " + str(
                            # tc.dag_id) + " to GPU " + str(device_id), "at", self.time_stamp  
                else:
                    self.ready_queue['gpu'].append(device_id)
                    self.mGPU += 1
            else:
                cpu = device_id
                if len(self.dispatch_queue['cpu']) > 0:
                    tc = self.dispatch_queue['cpu'].popleft()
                    self.nCPU += 1
                    # print "Popped from CPU queue", tc
                    self.dispatch_task_set(device_id,-1,tc,self.dags[tc.dag_id])
                    # print "SCHEDULE: Dispatching from CPU Queue" + str(list(tc.get_kernel_ids())) + " of DAG " + str(
                            # tc.dag_id) + " to CPU " + str(device_id), "at", self.time_stamp

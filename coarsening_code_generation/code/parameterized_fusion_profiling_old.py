    ...
    for k in range(len(cnn)):
        ...
        for cf in [1,2,4,8,16]:
            for ct in ['T','B']:
                ...
                if execution_map[json_name][2]>max_htd_overhead:
                    max_htd_overhead=execution_map[json_name][2]
                if execution_map[json_name][3]>max_dth_overhead:
...
def find_best_coarsened_version(node,execution_map,device):
    ...
    for cf in [1,2,4,8,16]:
        for ct in ['T','B']:
            ...
            if execution_map[json_name][0 if device=='cpu' else 1]<min_latency:
...
if __name__ == "__main__":
    ...
    file_name = "./dags/" + network_name + "_" + str(height) + "_dag.graph"

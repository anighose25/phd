__kernel void convolution_6( __global const float* b36, __global const float* b37, __global const float* b38, __global float* b39)
{
			...
			for(int i=0;i<3;i++)
			{
				...
				else if(inputCol_C<0)
				{
					if(inputRow_C+i==0 && j==0)
					{
						...
						data0_C.xyz = vload3(0,(__global const float *)b36+(j*128*128)+(inputRow_C+i)*128+inputCol_C);
					data0_C.x = 0;
				}
				else if(inputCol_C+3-1>=128)
				{
					...
					{
						data0_C.x = b36[(j*128*128)+(inputRow_C+i)*128+2];
						data0_C.y = b36[(j*128*128)+(inputRow_C+i)*128+3];
					}
					else
						data0_C.xyz = vload3(0,(__global const float *)b36+(j*128*128)+(inputRow_C+i)*128+inputCol_C);
					data0_C.z = 0;
				...
				data0_C.w = 0.0;
				data1_C.xyz = vload3(0,(__global const float *)b37+(plane_C*3*3*64)+(j*3*3)+i*3);
				dots += dot(data0_C,data1_C);
			}
		}
		if(outputRow_C<128 && outputCol_C<128)
			b39[plane_C*128*128+outputRow_C*128+outputCol_C] = dots + b38[plane_C];
	}

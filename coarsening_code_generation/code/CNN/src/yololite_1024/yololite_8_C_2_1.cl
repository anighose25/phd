__kernel void convolution_8( __global const float* b4, __global const float* b5, __global const float* b6, __global float* b7)
{
	...
	if(dp_0< 128*64*64)
	{
			...
			for(int i=0;i<3;i++)
			{
				...
				else if(inputCol_C<0)
				{
					if(inputRow_C+i==0 && j==0)
					{
						...
						data0_C.xyz = vload3(0,(__global const float *)b4+(j*64*64)+(inputRow_C+i)*64+inputCol_C);
					data0_C.x = 0;
				}
				else if(inputCol_C+3-1>=64)
				{
					...
					{
						data0_C.x = b4[(j*64*64)+(inputRow_C+i)*64+2];
						data0_C.y = b4[(j*64*64)+(inputRow_C+i)*64+3];
					}
					else
						data0_C.xyz = vload3(0,(__global const float *)b4+(j*64*64)+(inputRow_C+i)*64+inputCol_C);
					data0_C.z = 0;
		...
		if(outputRow_C<64 && outputCol_C<64)
			b7[plane_C*64*64+outputRow_C*64+outputCol_C] = dots + b6[plane_C];
	}

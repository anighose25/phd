__kernel void convolution_2( __global const float* b36, __global const float* b37, __global const float* b38, __global float* b39)
{
	...
	if(dp_0< 32*512*512)
	{
			...
			for(int i=0;i<3;i++)
			{
				...
				else if(inputCol_C<0)
				{
					if(inputRow_C+i==0 && j==0)
					{
						...
						data0_C.xyz = vload3(0,(__global const float *)b36+(j*512*512)+(inputRow_C+i)*512+inputCol_C);
					data0_C.x = 0;
				}
				else if(inputCol_C+3-1>=512)
				{
					...
					{
						data0_C.x = b36[(j*512*512)+(inputRow_C+i)*512+2];
						data0_C.y = b36[(j*512*512)+(inputRow_C+i)*512+3];
					}
					else
						data0_C.xyz = vload3(0,(__global const float *)b36+(j*512*512)+(inputRow_C+i)*512+inputCol_C);
					data0_C.z = 0;
		...
		if(outputRow_C<512 && outputCol_C<512)
			b39[plane_C*512*512+outputRow_C*512+outputCol_C] = dots + b38[plane_C];
	}

__kernel void convolution_6( __global const float* b32, __global const float* b33, __global const float* b34, __global float* b35)
{
	...
	if(dp_0< 128*128*128)
	{
			...
			for(int i=0;i<3;i++)
			{
				...
				else if(inputCol_C<0)
				{
					if(inputRow_C+i==0 && j==0)
					{
						...
						data0_C.xyz = vload3(0,(__global const float *)b32+(j*128*128)+(inputRow_C+i)*128+inputCol_C);
					data0_C.x = 0;
				}
				else if(inputCol_C+3-1>=128)
				{
					...
					{
						data0_C.x = b32[(j*128*128)+(inputRow_C+i)*128+2];
						data0_C.y = b32[(j*128*128)+(inputRow_C+i)*128+3];
					}
					else
						data0_C.xyz = vload3(0,(__global const float *)b32+(j*128*128)+(inputRow_C+i)*128+inputCol_C);
					data0_C.z = 0;
		...
		if(outputRow_C<128 && outputCol_C<128)
			b35[plane_C*128*128+outputRow_C*128+outputCol_C] = dots + b34[plane_C];
	}

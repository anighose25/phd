__kernel void convolution_10( __global const float* b8, __global const float* b9, __global const float* b10, __global float* b11)
{
	...
	if(dp_0< 256*32*32)
	{
			...
			for(int i=0;i<3;i++)
			{
				...
				else if(inputCol_C<0)
				{
					if(inputRow_C+i==0 && j==0)
					{
						...
						data0_C.xyz = vload3(0,(__global const float *)b8+(j*32*32)+(inputRow_C+i)*32+inputCol_C);
					data0_C.x = 0;
				}
				else if(inputCol_C+3-1>=32)
				{
					...
					{
						data0_C.x = b8[(j*32*32)+(inputRow_C+i)*32+2];
						data0_C.y = b8[(j*32*32)+(inputRow_C+i)*32+3];
					}
					else
						data0_C.xyz = vload3(0,(__global const float *)b8+(j*32*32)+(inputRow_C+i)*32+inputCol_C);
					data0_C.z = 0;
		...
		if(outputRow_C<32 && outputCol_C<32)
			b11[plane_C*32*32+outputRow_C*32+outputCol_C] = dots + b10[plane_C];
	}

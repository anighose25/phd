__kernel void convolution_0( __global const float* b36, __global const float* b37, __global const float* b38, __global float* b39)
{
			...
			for(int i=0;i<3;i++)
			{
				...
				else if(inputCol_C<0)
				{
					if(inputRow_C+i==0 && j==0)
					{
						...
						data0_C.xyz = vload3(0,(__global const float *)b36+(j*1024*1024)+(inputRow_C+i)*1024+inputCol_C);
					data0_C.x = 0;
				}
				else if(inputCol_C+3-1>=1024)
				{
					...
					{
						data0_C.x = b36[(j*1024*1024)+(inputRow_C+i)*1024+2];
						data0_C.y = b36[(j*1024*1024)+(inputRow_C+i)*1024+3];
					}
					else
						data0_C.xyz = vload3(0,(__global const float *)b36+(j*1024*1024)+(inputRow_C+i)*1024+inputCol_C);
					data0_C.z = 0;
				...
				data0_C.w = 0.0;
				data1_C.xyz = vload3(0,(__global const float *)b37+(plane_C*3*3*1)+(j*3*3)+i*3);
				dots += dot(data0_C,data1_C);
			}
		}
		if(outputRow_C<1024 && outputCol_C<1024)
			b39[plane_C*1024*1024+outputRow_C*1024+outputCol_C] = dots + b38[plane_C];
	}

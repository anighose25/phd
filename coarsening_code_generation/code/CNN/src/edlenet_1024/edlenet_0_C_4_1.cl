__kernel void convolution_0( __global const float* b8, __global const float* b9, __global const float* b10, __global float* b11)
{
			...
			for(int i=0;i<3;i++)
			{
				...
				else if(inputCol_C<0)
				{
					if(inputRow_C+i==0 && j==0)
					{
						...
						data0_C.xyz = vload3(0,(__global const float *)b8+(j*1024*1024)+(inputRow_C+i)*1024+inputCol_C);
					data0_C.x = 0;
				}
				else if(inputCol_C+3-1>=1024)
				{
					...
					{
						data0_C.x = b8[(j*1024*1024)+(inputRow_C+i)*1024+2];
						data0_C.y = b8[(j*1024*1024)+(inputRow_C+i)*1024+3];
					}
					else
						data0_C.xyz = vload3(0,(__global const float *)b8+(j*1024*1024)+(inputRow_C+i)*1024+inputCol_C);
					data0_C.z = 0;
				...
				data0_C.w = 0.0;
				data1_C.xyz = vload3(0,(__global const float *)b9+(plane_C*3*3*1)+(j*3*3)+i*3);
				dots += dot(data0_C,data1_C);
			}
		}
		if(outputRow_C<1024 && outputCol_C<1024)
			b11[plane_C*1024*1024+outputRow_C*1024+outputCol_C] = dots + b10[plane_C];
	}

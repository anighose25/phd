__kernel void pooling_3( __global const float* b18, __global float* b19)
{
	...
	if(dp_0< 64*256*256)
	{
		...
		inputImageOffset  = (n_P*64 + plane_P)*512*512;
		...
		if(process)
		{
			...
			data1_P = vload2(0,(__global const float *)b18+poolInputOffset+512);
	...
	if(dp_1< 64*256*256)
	{
		...
		localId_P = dp_1 % (256*256);
		outputRow_P = localId_P / 256;
		outputCol_P = localId_P % 256;
		image2dIdx_P = dp_1 / (256*256);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_2< 64*256*256)
	{
		...
		localId_P = dp_2 % (256*256);
		outputRow_P = localId_P / 256;
		outputCol_P = localId_P % 256;
		image2dIdx_P = dp_2 / (256*256);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_3< 64*256*256)
	{
		...
		localId_P = dp_3 % (256*256);
		outputRow_P = localId_P / 256;
		outputCol_P = localId_P % 256;
		image2dIdx_P = dp_3 / (256*256);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_4< 64*256*256)
	{
		...
		localId_P = dp_4 % (256*256);
		outputRow_P = localId_P / 256;
		outputCol_P = localId_P % 256;
		image2dIdx_P = dp_4 / (256*256);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_5< 64*256*256)
	{
		...
		localId_P = dp_5 % (256*256);
		outputRow_P = localId_P / 256;
		outputCol_P = localId_P % 256;
		image2dIdx_P = dp_5 / (256*256);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_6< 64*256*256)
	{
		...
		localId_P = dp_6 % (256*256);
		outputRow_P = localId_P / 256;
		outputCol_P = localId_P % 256;
		image2dIdx_P = dp_6 / (256*256);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_7< 64*256*256)
	{
		...
		localId_P = dp_7 % (256*256);
		outputRow_P = localId_P / 256;
		outputCol_P = localId_P % 256;
		image2dIdx_P = dp_7 / (256*256);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_8< 64*256*256)
	{
		...
		localId_P = dp_8 % (256*256);
		outputRow_P = localId_P / 256;
		outputCol_P = localId_P % 256;
		image2dIdx_P = dp_8 / (256*256);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_9< 64*256*256)
	{
		...
		localId_P = dp_9 % (256*256);
		outputRow_P = localId_P / 256;
		outputCol_P = localId_P % 256;
		image2dIdx_P = dp_9 / (256*256);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_10< 64*256*256)
	{
		...
		localId_P = dp_10 % (256*256);
		outputRow_P = localId_P / 256;
		outputCol_P = localId_P % 256;
		image2dIdx_P = dp_10 / (256*256);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_11< 64*256*256)
	{
		...
		localId_P = dp_11 % (256*256);
		outputRow_P = localId_P / 256;
		outputCol_P = localId_P % 256;
		image2dIdx_P = dp_11 / (256*256);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_12< 64*256*256)
	{
		...
		localId_P = dp_12 % (256*256);
		outputRow_P = localId_P / 256;
		outputCol_P = localId_P % 256;
		image2dIdx_P = dp_12 / (256*256);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_13< 64*256*256)
	{
		...
		localId_P = dp_13 % (256*256);
		outputRow_P = localId_P / 256;
		outputCol_P = localId_P % 256;
		image2dIdx_P = dp_13 / (256*256);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_14< 64*256*256)
	{
		...
		localId_P = dp_14 % (256*256);
		outputRow_P = localId_P / 256;
		outputCol_P = localId_P % 256;
		image2dIdx_P = dp_14 / (256*256);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_15< 64*256*256)
	{
		...
		localId_P = dp_15 % (256*256);
		outputRow_P = localId_P / 256;
		outputCol_P = localId_P % 256;
		image2dIdx_P = dp_15 / (256*256);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;

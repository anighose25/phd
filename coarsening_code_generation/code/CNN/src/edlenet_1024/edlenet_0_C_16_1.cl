__kernel void convolution_0( __global const float* b16, __global const float* b17, __global const float* b18, __global float* b19)
{
	...
	if(dp_0< 32*1024*1024)
	{
		...
		for(int j=0;j<1;j++)
		{
			for(int i=0;i<3;i++)
			{
				if(inputRow_C+i<0 || inputRow_C+i>=1024)
				...
				else if(inputCol_C<0)
				{
					if(inputRow_C+i==0 && j==0)
					{
						...
						data0_C.xyz = vload3(0,(__global const float *)b16+(j*1024*1024)+(inputRow_C+i)*1024+inputCol_C);
					data0_C.x = 0;
				}
				else if(inputCol_C+3-1>=1024)
				{
					if(inputRow_C+i==1024 && j==2)
					{
						data0_C.x = b16[(j*1024*1024)+(inputRow_C+i)*1024+2];
						data0_C.y = b16[(j*1024*1024)+(inputRow_C+i)*1024+3];
					}
					else
						data0_C.xyz = vload3(0,(__global const float *)b16+(j*1024*1024)+(inputRow_C+i)*1024+inputCol_C);
					data0_C.z = 0;
				...
				data0_C.w = 0.0;
				data1_C.xyz = vload3(0,(__global const float *)b17+(plane_C*3*3*1)+(j*3*3)+i*3);
				dots += dot(data0_C,data1_C);
			}
		}
		if(outputRow_C<1024 && outputCol_C<1024)
			b19[plane_C*1024*1024+outputRow_C*1024+outputCol_C] = dots + b18[plane_C];
	}

	if(dp_1< 32*1024*1024)
	{
		typedef float4 floatC;
		floatC data0_C,data1_C;
		int localId_C, w2 , h2, outputRow_C, outputCol_C, image2dIdx_C, plane_C, n_C, stride, pad, inputRow_C, inputCol_C;

__kernel void convolution_4( __global const float* b12, __global const float* b13, __global const float* b14, __global float* b15)
{
	...
	if(dp_0< 64*64*64)
	{
		...
		for(int j=0;j<32;j++)
		{
			for(int i=0;i<3;i++)
			{
				...
				data0_C.w = 0.0;
				data1_C.xyz = vload3(0,(__global const float *)b13+(plane_C*3*3*32)+(j*3*3)+i*3);

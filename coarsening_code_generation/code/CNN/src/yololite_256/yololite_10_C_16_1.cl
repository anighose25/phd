__kernel void convolution_10( __global const float* b16, __global const float* b17, __global const float* b18, __global float* b19)
{
	...
	if(dp_0< 256*8*8)
	{
			...
			for(int i=0;i<3;i++)
			{
				...
				else if(inputCol_C<0)
				{
					if(inputRow_C+i==0 && j==0)
					{
						...
						data0_C.xyz = vload3(0,(__global const float *)b16+(j*8*8)+(inputRow_C+i)*8+inputCol_C);
					data0_C.x = 0;
				}
				else if(inputCol_C+3-1>=8)
				{
					...
					{
						data0_C.x = b16[(j*8*8)+(inputRow_C+i)*8+2];
						data0_C.y = b16[(j*8*8)+(inputRow_C+i)*8+3];
					}
					else
						data0_C.xyz = vload3(0,(__global const float *)b16+(j*8*8)+(inputRow_C+i)*8+inputCol_C);
					data0_C.z = 0;
		...
		if(outputRow_C<8 && outputCol_C<8)
			b19[plane_C*8*8+outputRow_C*8+outputCol_C] = dots + b18[plane_C];
	}

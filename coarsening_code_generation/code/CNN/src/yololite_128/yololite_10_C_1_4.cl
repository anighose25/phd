__kernel void convolution_10( __global const float* b28, __global const float* b29, __global const float* b30, __global float* b31)
{
			...
			for(int i=0;i<3;i++)
			{
				...
				else if(inputCol_C<0)
				{
					if(inputRow_C+i==0 && j==0)
					{
						...
						data0_C.xyz = vload3(0,(__global const float *)b28+(j*4*4)+(inputRow_C+i)*4+inputCol_C);
					data0_C.x = 0;
				}
				else if(inputCol_C+3-1>=4)
				{
					...
					{
						data0_C.x = b28[(j*4*4)+(inputRow_C+i)*4+2];
						data0_C.y = b28[(j*4*4)+(inputRow_C+i)*4+3];
					}
					else
						data0_C.xyz = vload3(0,(__global const float *)b28+(j*4*4)+(inputRow_C+i)*4+inputCol_C);
					data0_C.z = 0;
				...
				data0_C.w = 0.0;
				data1_C.xyz = vload3(0,(__global const float *)b29+(plane_C*3*3*128)+(j*3*3)+i*3);
				dots += dot(data0_C,data1_C);
			}
		}
		if(outputRow_C<4 && outputCol_C<4)
			b31[plane_C*4*4+outputRow_C*4+outputCol_C] = dots + b30[plane_C];
	}

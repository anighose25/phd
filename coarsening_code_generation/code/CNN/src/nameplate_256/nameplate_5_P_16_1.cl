__kernel void pooling_5( __global const float* b8, __global float* b9)
{
	...
	if(dp_1< 64*32*16)
	{
		...
		localId_P = dp_1 % (32*16);
		outputRow_P = localId_P / 32;
		outputCol_P = localId_P % 16;
		image2dIdx_P = dp_1 / (32*16);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_2< 64*32*16)
	{
		...
		localId_P = dp_2 % (32*16);
		outputRow_P = localId_P / 32;
		outputCol_P = localId_P % 16;
		image2dIdx_P = dp_2 / (32*16);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_3< 64*32*16)
	{
		...
		localId_P = dp_3 % (32*16);
		outputRow_P = localId_P / 32;
		outputCol_P = localId_P % 16;
		image2dIdx_P = dp_3 / (32*16);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_4< 64*32*16)
	{
		...
		localId_P = dp_4 % (32*16);
		outputRow_P = localId_P / 32;
		outputCol_P = localId_P % 16;
		image2dIdx_P = dp_4 / (32*16);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_5< 64*32*16)
	{
		...
		localId_P = dp_5 % (32*16);
		outputRow_P = localId_P / 32;
		outputCol_P = localId_P % 16;
		image2dIdx_P = dp_5 / (32*16);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_6< 64*32*16)
	{
		...
		localId_P = dp_6 % (32*16);
		outputRow_P = localId_P / 32;
		outputCol_P = localId_P % 16;
		image2dIdx_P = dp_6 / (32*16);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_7< 64*32*16)
	{
		...
		localId_P = dp_7 % (32*16);
		outputRow_P = localId_P / 32;
		outputCol_P = localId_P % 16;
		image2dIdx_P = dp_7 / (32*16);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_8< 64*32*16)
	{
		...
		localId_P = dp_8 % (32*16);
		outputRow_P = localId_P / 32;
		outputCol_P = localId_P % 16;
		image2dIdx_P = dp_8 / (32*16);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_9< 64*32*16)
	{
		...
		localId_P = dp_9 % (32*16);
		outputRow_P = localId_P / 32;
		outputCol_P = localId_P % 16;
		image2dIdx_P = dp_9 / (32*16);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_10< 64*32*16)
	{
		...
		localId_P = dp_10 % (32*16);
		outputRow_P = localId_P / 32;
		outputCol_P = localId_P % 16;
		image2dIdx_P = dp_10 / (32*16);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_11< 64*32*16)
	{
		...
		localId_P = dp_11 % (32*16);
		outputRow_P = localId_P / 32;
		outputCol_P = localId_P % 16;
		image2dIdx_P = dp_11 / (32*16);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_12< 64*32*16)
	{
		...
		localId_P = dp_12 % (32*16);
		outputRow_P = localId_P / 32;
		outputCol_P = localId_P % 16;
		image2dIdx_P = dp_12 / (32*16);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_13< 64*32*16)
	{
		...
		localId_P = dp_13 % (32*16);
		outputRow_P = localId_P / 32;
		outputCol_P = localId_P % 16;
		image2dIdx_P = dp_13 / (32*16);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_14< 64*32*16)
	{
		...
		localId_P = dp_14 % (32*16);
		outputRow_P = localId_P / 32;
		outputCol_P = localId_P % 16;
		image2dIdx_P = dp_14 / (32*16);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;
	...
	if(dp_15< 64*32*16)
	{
		...
		localId_P = dp_15 % (32*16);
		outputRow_P = localId_P / 32;
		outputCol_P = localId_P % 16;
		image2dIdx_P = dp_15 / (32*16);
		plane_P = image2dIdx_P % 64;
		n_P = image2dIdx_P / 64;
		if(n_P > 1) 
			return;

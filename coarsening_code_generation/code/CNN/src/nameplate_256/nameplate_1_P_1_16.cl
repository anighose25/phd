__kernel void pooling_1( __global const float* b18, __global float* b19)
{
	...
	if(dp_1< 16*128*64)
	{
		...
		localId_P = dp_1 % (128*64);
		outputRow_P = localId_P / 128;
		outputCol_P = localId_P % 64;
		image2dIdx_P = dp_1 / (128*64);
		plane_P = image2dIdx_P % 16;
		n_P = image2dIdx_P / 16;
		if(n_P > 1) 
			return;
	...
	if(dp_2< 16*128*64)
	{
		...
		localId_P = dp_2 % (128*64);
		outputRow_P = localId_P / 128;
		outputCol_P = localId_P % 64;
		image2dIdx_P = dp_2 / (128*64);
		plane_P = image2dIdx_P % 16;
		n_P = image2dIdx_P / 16;
		if(n_P > 1) 
			return;
	...
	if(dp_3< 16*128*64)
	{
		...
		localId_P = dp_3 % (128*64);
		outputRow_P = localId_P / 128;
		outputCol_P = localId_P % 64;
		image2dIdx_P = dp_3 / (128*64);
		plane_P = image2dIdx_P % 16;
		n_P = image2dIdx_P / 16;
		if(n_P > 1) 
			return;
	...
	if(dp_4< 16*128*64)
	{
		...
		localId_P = dp_4 % (128*64);
		outputRow_P = localId_P / 128;
		outputCol_P = localId_P % 64;
		image2dIdx_P = dp_4 / (128*64);
		plane_P = image2dIdx_P % 16;
		n_P = image2dIdx_P / 16;
		if(n_P > 1) 
			return;
	...
	if(dp_5< 16*128*64)
	{
		...
		localId_P = dp_5 % (128*64);
		outputRow_P = localId_P / 128;
		outputCol_P = localId_P % 64;
		image2dIdx_P = dp_5 / (128*64);
		plane_P = image2dIdx_P % 16;
		n_P = image2dIdx_P / 16;
		if(n_P > 1) 
			return;
	...
	if(dp_6< 16*128*64)
	{
		...
		localId_P = dp_6 % (128*64);
		outputRow_P = localId_P / 128;
		outputCol_P = localId_P % 64;
		image2dIdx_P = dp_6 / (128*64);
		plane_P = image2dIdx_P % 16;
		n_P = image2dIdx_P / 16;
		if(n_P > 1) 
			return;
	...
	if(dp_7< 16*128*64)
	{
		...
		localId_P = dp_7 % (128*64);
		outputRow_P = localId_P / 128;
		outputCol_P = localId_P % 64;
		image2dIdx_P = dp_7 / (128*64);
		plane_P = image2dIdx_P % 16;
		n_P = image2dIdx_P / 16;
		if(n_P > 1) 
			return;
	...
	if(dp_8< 16*128*64)
	{
		...
		localId_P = dp_8 % (128*64);
		outputRow_P = localId_P / 128;
		outputCol_P = localId_P % 64;
		image2dIdx_P = dp_8 / (128*64);
		plane_P = image2dIdx_P % 16;
		n_P = image2dIdx_P / 16;
		if(n_P > 1) 
			return;
	...
	if(dp_9< 16*128*64)
	{
		...
		localId_P = dp_9 % (128*64);
		outputRow_P = localId_P / 128;
		outputCol_P = localId_P % 64;
		image2dIdx_P = dp_9 / (128*64);
		plane_P = image2dIdx_P % 16;
		n_P = image2dIdx_P / 16;
		if(n_P > 1) 
			return;
	...
	if(dp_10< 16*128*64)
	{
		...
		localId_P = dp_10 % (128*64);
		outputRow_P = localId_P / 128;
		outputCol_P = localId_P % 64;
		image2dIdx_P = dp_10 / (128*64);
		plane_P = image2dIdx_P % 16;
		n_P = image2dIdx_P / 16;
		if(n_P > 1) 
			return;
	...
	if(dp_11< 16*128*64)
	{
		...
		localId_P = dp_11 % (128*64);
		outputRow_P = localId_P / 128;
		outputCol_P = localId_P % 64;
		image2dIdx_P = dp_11 / (128*64);
		plane_P = image2dIdx_P % 16;
		n_P = image2dIdx_P / 16;
		if(n_P > 1) 
			return;
	...
	if(dp_12< 16*128*64)
	{
		...
		localId_P = dp_12 % (128*64);
		outputRow_P = localId_P / 128;
		outputCol_P = localId_P % 64;
		image2dIdx_P = dp_12 / (128*64);
		plane_P = image2dIdx_P % 16;
		n_P = image2dIdx_P / 16;
		if(n_P > 1) 
			return;
	...
	if(dp_13< 16*128*64)
	{
		...
		localId_P = dp_13 % (128*64);
		outputRow_P = localId_P / 128;
		outputCol_P = localId_P % 64;
		image2dIdx_P = dp_13 / (128*64);
		plane_P = image2dIdx_P % 16;
		n_P = image2dIdx_P / 16;
		if(n_P > 1) 
			return;
	...
	if(dp_14< 16*128*64)
	{
		...
		localId_P = dp_14 % (128*64);
		outputRow_P = localId_P / 128;
		outputCol_P = localId_P % 64;
		image2dIdx_P = dp_14 / (128*64);
		plane_P = image2dIdx_P % 16;
		n_P = image2dIdx_P / 16;
		if(n_P > 1) 
			return;
	...
	if(dp_15< 16*128*64)
	{
		...
		localId_P = dp_15 % (128*64);
		outputRow_P = localId_P / 128;
		outputCol_P = localId_P % 64;
		image2dIdx_P = dp_15 / (128*64);
		plane_P = image2dIdx_P % 16;
		n_P = image2dIdx_P / 16;
		if(n_P > 1) 
			return;

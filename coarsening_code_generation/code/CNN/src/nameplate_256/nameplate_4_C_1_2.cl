__kernel void convolution_4( __global const float* b24, __global const float* b25, __global const float* b26, __global float* b27)
{
	...
	if(dp_0< 64*64*32)
	{
			...
			for(int i=0;i<3;i++)
			{
				...
				else if(inputCol_C<0)
				{
					if(inputRow_C+i==0 && j==0)
					{
						...
						data0_C.xyz = vload3(0,(__global const float *)b24+(j*64*32)+(inputRow_C+i)*64+inputCol_C);
					data0_C.x = 0;
				}
				else if(inputCol_C+3-1>=32)
				{
					...
					{
						data0_C.x = b24[(j*64*32)+(inputRow_C+i)*64+2];
						data0_C.y = b24[(j*64*32)+(inputRow_C+i)*64+3];
					}
					else
						data0_C.xyz = vload3(0,(__global const float *)b24+(j*64*32)+(inputRow_C+i)*64+inputCol_C);
					data0_C.z = 0;
		...
		if(outputRow_C<32 && outputCol_C<32)
			b27[plane_C*32*64+outputRow_C*64+outputCol_C] = dots + b26[plane_C];
	}

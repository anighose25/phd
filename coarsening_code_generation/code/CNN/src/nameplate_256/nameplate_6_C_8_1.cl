__kernel void convolution_6( __global const float* b12, __global const float* b13, __global const float* b14, __global float* b15)
{
	...
	if(dp_0< 128*32*16)
	{
			...
			for(int i=0;i<3;i++)
			{
				...
				else if(inputCol_C<0)
				{
					if(inputRow_C+i==0 && j==0)
					{
						...
						data0_C.xyz = vload3(0,(__global const float *)b12+(j*32*16)+(inputRow_C+i)*32+inputCol_C);
					data0_C.x = 0;
				}
				else if(inputCol_C+3-1>=16)
				{
					...
					{
						data0_C.x = b12[(j*32*16)+(inputRow_C+i)*32+2];
						data0_C.y = b12[(j*32*16)+(inputRow_C+i)*32+3];
					}
					else
						data0_C.xyz = vload3(0,(__global const float *)b12+(j*32*16)+(inputRow_C+i)*32+inputCol_C);
					data0_C.z = 0;
		...
		if(outputRow_C<16 && outputCol_C<16)
			b15[plane_C*16*32+outputRow_C*32+outputCol_C] = dots + b14[plane_C];
	}

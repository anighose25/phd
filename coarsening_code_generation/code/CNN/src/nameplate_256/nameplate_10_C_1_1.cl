__kernel void convolution_10( __global const float* b20, __global const float* b21, __global const float* b22, __global float* b23)
{
	...
	if(dp_0< 4*8*4)
	{
			...
			for(int i=0;i<3;i++)
			{
				...
				else if(inputCol_C<0)
				{
					if(inputRow_C+i==0 && j==0)
					{
						...
						data0_C.xyz = vload3(0,(__global const float *)b20+(j*8*4)+(inputRow_C+i)*8+inputCol_C);
					data0_C.x = 0;
				}
				else if(inputCol_C+3-1>=4)
				{
					...
					{
						data0_C.x = b20[(j*8*4)+(inputRow_C+i)*8+2];
						data0_C.y = b20[(j*8*4)+(inputRow_C+i)*8+3];
					}
					else
						data0_C.xyz = vload3(0,(__global const float *)b20+(j*8*4)+(inputRow_C+i)*8+inputCol_C);
					data0_C.z = 0;
		...
		if(outputRow_C<4 && outputCol_C<4)
			b23[plane_C*4*8+outputRow_C*8+outputCol_C] = dots + b22[plane_C];
	}

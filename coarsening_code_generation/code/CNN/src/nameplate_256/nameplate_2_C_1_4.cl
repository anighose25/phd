__kernel void convolution_2( __global const float* b28, __global const float* b29, __global const float* b30, __global float* b31)
{
	...
	if(dp_0< 32*128*64)
	{
			...
			for(int i=0;i<3;i++)
			{
				...
				else if(inputCol_C<0)
				{
					if(inputRow_C+i==0 && j==0)
					{
						...
						data0_C.xyz = vload3(0,(__global const float *)b28+(j*128*64)+(inputRow_C+i)*128+inputCol_C);
					data0_C.x = 0;
				}
				else if(inputCol_C+3-1>=64)
				{
					...
					{
						data0_C.x = b28[(j*128*64)+(inputRow_C+i)*128+2];
						data0_C.y = b28[(j*128*64)+(inputRow_C+i)*128+3];
					}
					else
						data0_C.xyz = vload3(0,(__global const float *)b28+(j*128*64)+(inputRow_C+i)*128+inputCol_C);
					data0_C.z = 0;
		...
		if(outputRow_C<64 && outputCol_C<64)
			b31[plane_C*64*128+outputRow_C*128+outputCol_C] = dots + b30[plane_C];
	}

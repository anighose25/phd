__kernel void pooling_3( __global const float* b18, __global float* b19)
{
	...
	if(dp_1< 32*64*32)
	{
		...
		localId_P = dp_1 % (64*32);
		outputRow_P = localId_P / 64;
		outputCol_P = localId_P % 32;
		image2dIdx_P = dp_1 / (64*32);
		plane_P = image2dIdx_P % 32;
		n_P = image2dIdx_P / 32;
		if(n_P > 1) 
			return;
	...
	if(dp_2< 32*64*32)
	{
		...
		localId_P = dp_2 % (64*32);
		outputRow_P = localId_P / 64;
		outputCol_P = localId_P % 32;
		image2dIdx_P = dp_2 / (64*32);
		plane_P = image2dIdx_P % 32;
		n_P = image2dIdx_P / 32;
		if(n_P > 1) 
			return;
	...
	if(dp_3< 32*64*32)
	{
		...
		localId_P = dp_3 % (64*32);
		outputRow_P = localId_P / 64;
		outputCol_P = localId_P % 32;
		image2dIdx_P = dp_3 / (64*32);
		plane_P = image2dIdx_P % 32;
		n_P = image2dIdx_P / 32;
		if(n_P > 1) 
			return;
	...
	if(dp_4< 32*64*32)
	{
		...
		localId_P = dp_4 % (64*32);
		outputRow_P = localId_P / 64;
		outputCol_P = localId_P % 32;
		image2dIdx_P = dp_4 / (64*32);
		plane_P = image2dIdx_P % 32;
		n_P = image2dIdx_P / 32;
		if(n_P > 1) 
			return;
	...
	if(dp_5< 32*64*32)
	{
		...
		localId_P = dp_5 % (64*32);
		outputRow_P = localId_P / 64;
		outputCol_P = localId_P % 32;
		image2dIdx_P = dp_5 / (64*32);
		plane_P = image2dIdx_P % 32;
		n_P = image2dIdx_P / 32;
		if(n_P > 1) 
			return;
	...
	if(dp_6< 32*64*32)
	{
		...
		localId_P = dp_6 % (64*32);
		outputRow_P = localId_P / 64;
		outputCol_P = localId_P % 32;
		image2dIdx_P = dp_6 / (64*32);
		plane_P = image2dIdx_P % 32;
		n_P = image2dIdx_P / 32;
		if(n_P > 1) 
			return;
	...
	if(dp_7< 32*64*32)
	{
		...
		localId_P = dp_7 % (64*32);
		outputRow_P = localId_P / 64;
		outputCol_P = localId_P % 32;
		image2dIdx_P = dp_7 / (64*32);
		plane_P = image2dIdx_P % 32;
		n_P = image2dIdx_P / 32;
		if(n_P > 1) 
			return;
	...
	if(dp_8< 32*64*32)
	{
		...
		localId_P = dp_8 % (64*32);
		outputRow_P = localId_P / 64;
		outputCol_P = localId_P % 32;
		image2dIdx_P = dp_8 / (64*32);
		plane_P = image2dIdx_P % 32;
		n_P = image2dIdx_P / 32;
		if(n_P > 1) 
			return;
	...
	if(dp_9< 32*64*32)
	{
		...
		localId_P = dp_9 % (64*32);
		outputRow_P = localId_P / 64;
		outputCol_P = localId_P % 32;
		image2dIdx_P = dp_9 / (64*32);
		plane_P = image2dIdx_P % 32;
		n_P = image2dIdx_P / 32;
		if(n_P > 1) 
			return;
	...
	if(dp_10< 32*64*32)
	{
		...
		localId_P = dp_10 % (64*32);
		outputRow_P = localId_P / 64;
		outputCol_P = localId_P % 32;
		image2dIdx_P = dp_10 / (64*32);
		plane_P = image2dIdx_P % 32;
		n_P = image2dIdx_P / 32;
		if(n_P > 1) 
			return;
	...
	if(dp_11< 32*64*32)
	{
		...
		localId_P = dp_11 % (64*32);
		outputRow_P = localId_P / 64;
		outputCol_P = localId_P % 32;
		image2dIdx_P = dp_11 / (64*32);
		plane_P = image2dIdx_P % 32;
		n_P = image2dIdx_P / 32;
		if(n_P > 1) 
			return;
	...
	if(dp_12< 32*64*32)
	{
		...
		localId_P = dp_12 % (64*32);
		outputRow_P = localId_P / 64;
		outputCol_P = localId_P % 32;
		image2dIdx_P = dp_12 / (64*32);
		plane_P = image2dIdx_P % 32;
		n_P = image2dIdx_P / 32;
		if(n_P > 1) 
			return;
	...
	if(dp_13< 32*64*32)
	{
		...
		localId_P = dp_13 % (64*32);
		outputRow_P = localId_P / 64;
		outputCol_P = localId_P % 32;
		image2dIdx_P = dp_13 / (64*32);
		plane_P = image2dIdx_P % 32;
		n_P = image2dIdx_P / 32;
		if(n_P > 1) 
			return;
	...
	if(dp_14< 32*64*32)
	{
		...
		localId_P = dp_14 % (64*32);
		outputRow_P = localId_P / 64;
		outputCol_P = localId_P % 32;
		image2dIdx_P = dp_14 / (64*32);
		plane_P = image2dIdx_P % 32;
		n_P = image2dIdx_P / 32;
		if(n_P > 1) 
			return;
	...
	if(dp_15< 32*64*32)
	{
		...
		localId_P = dp_15 % (64*32);
		outputRow_P = localId_P / 64;
		outputCol_P = localId_P % 32;
		image2dIdx_P = dp_15 / (64*32);
		plane_P = image2dIdx_P % 32;
		n_P = image2dIdx_P / 32;
		if(n_P > 1) 
			return;

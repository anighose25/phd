__kernel void convolution_6( __global const float* b4, __global const float* b5, __global const float* b6, __global float* b7)
{
	...
	if(dp_0< 128*4*4)
	{
		...
		for(int j=0;j<64;j++)
		{
			for(int i=0;i<3;i++)
			{
				...
				data0_C.w = 0.0;
				data1_C.xyz = vload3(0,(__global const float *)b5+(plane_C*3*3*64)+(j*3*3)+i*3);

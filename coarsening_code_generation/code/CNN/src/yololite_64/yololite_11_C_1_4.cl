__kernel void convolution_11( __global const float* b28, __global const float* b29, __global const float* b30, __global float* b31)
{
	...
	if(dp_0< 125*2*2)
	{
		...
		for(int j=0;j<256;j++)
		{
			for(int i=0;i<3;i++)
			{
				...
				data0_C.w = 0.0;
				data1_C.xyz = vload3(0,(__global const float *)b29+(plane_C*3*3*256)+(j*3*3)+i*3);

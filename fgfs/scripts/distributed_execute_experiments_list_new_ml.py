...
if __name__ == '__main__':
    ...
    makespan_list_file = open("Makespans/makespans_list_ml_" +str(rank) + ".stats", 'w')
    # makespan_lc_file = open("Makespans/makespans_lc_" +str(rank) + ".stats", 'w')
    # errors_lc_file = open("ERRORS/error_lc_" + str(rank)+ ".stats", 'w')
    errors_list_file = open("ERRORS/error_list_" + str(rank)+ ".stats", 'w')
    graphs = open(file_name, 'r').readlines()
    DC = DAGCreator()
    contraction_depth = sys.argv[2]
    accuracy_list_file = open("Makespans/accuracy_list_" +str(rank) + ".stats", 'w')
        ...
        for rank in ["blevel"]:
            ...
            for graph in graphs: 
                try:
                    ...
                    dag_list.modify_partition_classes("GraphsNewAccuracy/accuracy_"+graph_name[10:][:-6]+".stats")
                    ...
                    accuracy_list_file.write(filename_list + ": " + str(dag_list.accuracy) + "\n")  
                    ...
                    # SE_lc.schedule_linear_clusters(dag_objects_lc, 2)
                    
                    
                    # graph_file = graph_name.split("/")[1]
                    # filename_lc = rank + "_" + "-1" + "_lc_" + str(num_devices) + "_" + graph_file[:-6]
                    # makespan_lc_file.write(filename_lc + ": " + str(SE_lc.time_stamp) + "\n")
                except:
                    graph_name = graph.split(":")[0]
                    errors_list_file.write(graph_name +  " " + str(num_devices) +  "\n")
